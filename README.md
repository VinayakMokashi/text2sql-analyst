...
> **Rock generates the highest revenue at $826.65, followed by Latin ($382.14), Metal
> ($261.36), Alternative & Punk ($241.56), and TV Shows ($93.53).**
>
> - Rock accounts for 45.8% of the total revenue generated by these top five genres.
> - The revenue gap between the top genre (Rock) and the second (Latin) is more than
>   double.
...
Tables kept by the LLM: Genre, Track, InvoiceLine. Total time: 2.3 s.
</details>
...
7. **Answer and analysis** ([`analysis/answer.py`](src/text2sql/analysis/answer.py)).
   ...
   the application** (totals, averages, extremes, the share of the top item and of the
   top three, the gap between highest and lowest, and the first-to-last change of a
   time series). LLMs are weak at arithmetic over many rows, so the model is asked to
   quote these exact numbers rather than compute them. It returns a direct answer, up to three insights
   and any caveats.
...
![SQL and how it was produced](docs/images/sql-details.png)
